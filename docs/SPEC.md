...
| LNemail | **A sixth provider, last in the chooser, reached over its own bearer-token REST API rather than IMAP or JMAP.** A disposable mailbox paid for and read over the Lightning Network: no address to type, since LNemail names its own, and no password — a bearer token, either pasted from an existing account and verified by asking `GET /account` whose it is, or freshly issued once a signup invoice this only displays and polls is paid. The account id is `lnemail:<address>`, the address itself learned the same way Gmail's is rather than typed. Its capability ceiling is the narrowest of the six: `batch` and `send` alone — no labels, no archive, no star, no search, no conversations, since the API offers none of them and a button that promised one would fail the moment it was pressed. **Sent is not absent, it is honest.** LNemail keeps no sent archive, only `GET /email/sends/recent` — a status log of the last ten payments, recipient and subject and whether each was paid and delivered, never a body — so the Sent mailbox shows that log and opening a row says plainly that no copy exists rather than showing an empty reader. The account itself expires — a year bought for 1000 sats — so a header control shows every connected mailbox's own days-until-expiry and opens a renewal invoice the same shape the signup one is, paid out of band and only ever displayed and polled here. |
...
- Six kinds of mailbox, in the order the chooser offers them: Gmail, Outlook, HEY and JMAP by name, IMAP for every server the first four do not, and LNemail for a mailbox paid for over the Lightning Network
