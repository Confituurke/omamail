...
- A mailbox is a **provider**: `gmail`, `hey`, `outlook`, `imap`, `jmap`, or `lnemail`, listed in that order — Gmail, HEY and Outlook first, the three hosted mailboxes with a service of their own, then IMAP and JMAP, the two that are every other mailbox, and LNemail last of all because it names one specific service rather than a protocol anything else could also speak, so it is met only once every named alternative has already been offered. `Registry.js` (assembled out of `Gmail.js`, `Hey.js`, `Outlook.js`, `Imap.js`, `Jmap.js` and `Lnemail.js`) is the only place that knows the differences — which mailboxes exist, what a query string means, what the service can be asked to do, and how it signs in. Nothing above it branches on a provider id.
- Two objects make a provider work: something that signs in (`AuthManager`, `OutlookAuth`, `HeyAuth`, `JmapAuth`, `ImapAuth`, `LnemailAuth`) and something that fetches (`GmailApiClient`, `HeyClient`, `JmapClient`, `ImapClient`, `LnemailClient`). `MailAccount` builds one pair through a `Loader` and drives them through an identical interface — same method names, same arguments, same callback shape. Adding a provider is those two files and a registry entry.
- **Every client hands back Gmail's message resource**: a headers array, a MIME
  ...
  *composes* the resource from a posting and a thread read rather than parsing
  one — same shape, no parse. LNemail composes too, and does it in Rust rather
  than in `LnemailClient.qml`: `providers::lnemail_resource` builds the
  resource directly from LNemail's own JSON, since there is no wire format
  underneath it to parse either — the direction mail fetching and parsing are
  migrating in generally (see `docs/BACKEND.md`), reached here first because
  there was nothing already in QML worth keeping.
...
- LNemail speaks neither IMAP nor JMAP: a bearer-token REST API of its own, `https://lnemail.net/api/v1`, fixed as an origin constant in `providers::lnemail_http` rather than ever taken from a caller. `providers::lnemail` is a thin, validating proxy over it — every method checks its own exact parameter set before a token is resolved or a request is built, and every response is either passed straight through or composed by `providers::lnemail_resource` into the shared message shape. Nothing here shells out and nothing here holds a session: the token is LNemail's only credential, never refreshed or rotated, so there is no state to keep beyond the HTTP transport itself.
- **Three calls take no account and no token at all**: creating a mailbox, checking whether its signup invoice has been paid, and reissuing one from a different provider all run before an account is registered — there is nothing yet for `accountId` to resolve through the keyring. `lnemail.account` is the one call that runs both ways: an explicit `token`, for verifying a pasted or freshly paid-for one before the account exists, or the keyring's own once it does. `account_token` is deliberately the only function that ever reaches `explicit_token`; `token`, the resolver every other authenticated method calls, refuses an explicit token outright rather than accepting one nobody asked it to — an invariant enforced by which function can call which, not by every other method's parameter allowlist happening to leave `token` out.
- **Sent is a status log, not an archive.** LNemail keeps no copy of what you sent, only `GET /email/sends/recent` — the last ten payments, by recipient, subject, and whether each was paid and delivered. `lnemail.list` reads it instead of the inbox when asked with the `sent` query (resolved the same way `unread` already is, in `providers::domain_queries.json`), and each row names itself `send-<payment_hash>` — a prefix that can never collide with a real message id, since the two are separate namespaces LNemail itself keeps apart, but nothing declares that except this string. `lnemail.read` recognises the prefix and pulls the matching entry back out of that same short list rather than asking an endpoint that does not exist for one send by itself; opening one composes a body that says plainly there is nothing to show rather than an empty reader. Deleting a sent row, or asking it for an attachment, is refused outright — `lnemail_sent_log_readonly` and `lnemail_not_found` respectively — rather than sent to an endpoint that was never going to recognise the id.
- **Send takes the same raw message every provider is handed**, not the structured fields LNemail's own endpoint wants. `providers::lnemail_resource::decode_outgoing` takes it apart with `mailparse`: the one recipient LNemail's endpoint accepts — refusing more than one named across To, Cc and Bcc outright, since sending to the first and dropping the rest is mail that never reaches someone the sender named — the first `text/plain` part (LNemail sends no HTML), and every attachment part re-encoded to the base64 its own JSON API wants — capped at 20 attachments and 8MB decoded between them, the same discipline `lnemail_http` already holds the response side to.
- LNemail accounts expire — 1000 sats buys a year — and `lnemail.account`'s answer already named `expires_at`, `is_expired`, `days_until_expiry` and `renewal_eligible` before anything here read them. `LnemailAuth` holds them as `statusChecked`/`daysUntilExpiry`/etc., read once at sign-in (folded into the same call that verifies the token, not a second round trip) and again whenever the renewal control is opened — never on a timer, since nothing needs to be fresher than the last time someone looked. Renewing is the same invoice-then-poll shape signing up already is: `lnemail.renewalInvoice` (`POST /account/renew`), displayed and polled by `lnemail.renewalStatus`, never touched by a wallet this holds itself. The header's own renewal control (a lightning-bolt icon before *Check mail*) is visible looking at one LNemail mailbox on its own, or at every mailbox at once with at least one LNemail account among them — `App.hasLnemailInView` — and opens `LnemailRenewalPopup`, a snapshot of every connected LNemail mailbox's own expiry taken when the popup opens rather than kept live.
- The keyring kind is `lnemail-token`, alongside `imap-password` and `jmap-secret` in `auth::credentials::password` and `Credentials.js`'s own `LNEMAIL_KEYRING_KIND`. A brand new mailbox has no account id until a profile read reports the address LNemail chose and `MailAccount` assigns one after the fact — the same way Gmail's own address is learned rather than typed — so `LnemailAuth` holds a freshly signed-in token under `unnamedToken` until that id lands, the same fix `AuthManager`'s own `unnamedRefreshToken` exists for: storing it immediately would write it under a name-less key any other LNemail mailbox could then find.
